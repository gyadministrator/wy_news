...
/*     
  ...
  * @Description:    
 */
class EmptyMusicRecyclerView : FrameLayout {
    private lateinit var rvContent: MusicRecyclerView
    private lateinit var rlEmpty: RelativeLayout
    private var loadListener: OnReloadListener? = null

    interface OnReloadListener {
        fun onReload()
    }

    fun setLoadListener(listener: OnReloadListener) {
    ...
    private fun initView(binding: LayoutEmptyRecyclerViewBinding) {
        ...
        //initRecycler()
        rlEmpty.setOnClickListener {
            loadListener?.onReload()
        }
    }

    fun getRecyclerView(): MusicRecyclerView {
        return rvContent
    }

    fun showData(hasData: Boolean) {
        if (hasData) {
